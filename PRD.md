...
- Functionality: Configurable client authentication mode switch with two modes applied consistently across both stacks. Mode 1 (`simple`) uses username/password authentication with local users and roles. Mode 2 (`entra`) uses Microsoft Entra ID authentication with application-specific Entra security groups mapped to roles. Supported roles are admin, recruiter, and analytics viewer. Recruiters are scoped to assigned organization and department. Admins can access all organizations and departments. Analytics viewers have read-only analytics access at global, organization, and organization+department filter levels.
- Purpose: Provide a production-ready authentication model that supports both current demo workflows and Entra-based enterprise identity while preserving identical authorization outcomes across Stack A and Stack B.
- Trigger: Application load or session timeout
- Progression: App load → Evaluate configured client auth mode → Authenticate with username/password (`simple`) or Entra sign-in (`entra`) → Resolve role from local role assignment (`simple`) or app-specific Entra group mapping (`entra`) → Render role-scoped experience → Logout and clear local session state.
- Success criteria: Auth mode behavior and role authorization are identical across both stacks in the same environment; admins can access all organizations/departments; recruiters are restricted to assigned organization and department; analytics viewers can view analytics at global, organization, and organization+department levels but cannot perform mutating actions; in `simple` mode local password operations remain secure with hashed storage; in `entra` mode local password management is disabled.
